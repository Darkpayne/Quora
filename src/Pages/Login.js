...
import { REQUEST_URI } from "../api/Request_URI";
...
const Login = () => {
  ...
  const NextVerification = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post(REQUEST_URI + "/api/auth/register", {
        name,
        email,
      });
  ...
  const ShowlastModal = async (e) => {
    ...
    setIsLoading(true);
    try {
      await axios.post(REQUEST_URI + "/api/auth/verify", {
        otp: OTP,
        id: emailOTP?.id,
      });
  ...
  const LoginCompleted = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post(REQUEST_URI + "/api/auth/password", {
        password: password,
        id: emailOTP?.id,
      });
  ...
  const handleLogin = async (e) => {
    ...
    if (loginEmail === "" || loginPassword === "") {
      ...
      try {
        const res = await axios.post(REQUEST_URI + "/api/auth/login", {
          email: loginEmail,
    ...
    >
      <div className="bg-white w-[700px] h-[548px]">
        <div className="p-4 mb-2">
          <h1 className="headingQuoras text-3xl text-center text-red-700 my-5">
            Quora
          </h1>
        ...
        <div className="flex border-b">
          ...
          <div className="basis-1/2 px-5 border-l mb-5">
            <h2 className="font-bold text-gray-500 border-b pb-2 mb-3">
              Login
            </h2>
            <form onSubmit={handleLogin}>
              ...
              <div className="mb-6 flex justify-between items-center">
                <span
                  ...
                  className="text-sm text-gray-400 focus:outline-none focus:text-indigo-500 hover:text-indigo-500 cursor-pointer dark:hover:text-indigo-300"
                >
                  Forgot password?
                </span>
                <button
                  type="submit"
                  className=" px-5 py-1 text-white bg-indigo-500 rounded-full focus:bg-indigo-600 focus:outline-none"
                >
                  Login
                </button>
              </div>
      ...
      <div
        ...
        className={`${
          !showModal ? "hidden" : ""
        } bg-slate-800 bg-opacity-90 flex justify-center items-center transition-all ease-in-out top-0 right-0 bottom-0 left-0 z-50 h-screen fixed`}
      >
        <div
          className={`relative p-4 w-full max-w-2xl h-full md:h-auto transition-all ease-in-out ${
          ...
          <div className="relative bg-white rounded-lg shadow">
            <div className="flex px-3 pt-2 space-x-2 items-center rounded-t ">
              <button
                ...
                type="button"
                className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-full text-xl p-1.5 inline-flex items-center "
            ...
            <section
              className={`${showSignup && !isLoading ? "block" : "hidden"}`}
            >
              ...
              <form onSubmit={NextVerification}>
                ...
                <div className=" flex justify-end items-center rounded-b border-t space-x-2 py-2 px-4">
                  <button
                    ...
                    type="submit"
                    className="text-white bg-blue-500 hover:bg-blue-800 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center"
                  >
                    Next
                  </button>
                </div>
            ...
            <section
              className={`${
                showEmailVerification && !isLoading ? "block" : "hidden"
              }`}
            >
              <h2 className="px-5 mt-3 text-2xl">Confirm your email</h2>
              <p className="px-5 mt-3 text-sm">
                Please enter the code we sent to {emailOTP?.email}!{" "}
                {`Your OTP is ${emailOTP?.otp}`}.
              </p>
              ...
              <form onSubmit={ShowlastModal}>
                ...
                <p className="text-xs px-5 mb-10 text-gray-500 hover:underline cursor-pointer">
                  Didn't recieve an email or something went wrong? Resend Code
                </p>
                {/* Buttons */}
            ...
            >
              <img
                src="https://media0.giphy.com/media/3oEjI6SIIHBdRxXI40/200w.gif?cid=82a1493b26cxzncu0dda6cfx2j74jkxhq9c2ikqd2vugnog0&rid=200w.gif&ct=g"
                alt=""
              />
            </section>

            <section
              className={`${forgetPassword && !isLoading ? "block" : "hidden"}`}
            >
            ...
            <section
              className={`${
                showPassVerification && !isLoading ? "block" : "hidden"
              }`}
            >
              <h2 className="px-5 mt-3 text-2xl">Sign Up</h2>
              <form onSubmit={LoginCompleted}>
                <div className="mb-8  mt-5">
                  <label
                    htmlFor="password"
                    className=" mb-2 font-medium  text-gray-900 flex justify-between px-5"
                  >
                    <span className="flex">
                      <span className="first-letter:">Password</span>

                      <span
                        className={`text-red-500 text-2xl flex font-extrabold ${
                          validPass || !password ? "hidden" : "inline"
                        }`}
                      >
                      ...
                      <span
                        className={`text-green-500 text-2xl flex font-extrabold ${
                          validPass ? "valid" : "hidden"
                        }`}
                      >
                  ...
                  <p
                    ref={errRef}
                    className={`mt-2 text-sm font-semibold text text-red-600 px-5 mb-10  ${
                      passFocus && !validPass ? "flex" : "hidden"
                    } flex items-start`}
                  >
                    <span className="flex mr-2 mt-1  font-semibold">
