    ...
    <>
      <main className='bg-white pt-2 px-1 border'>
        ...
        <div className="flex my-2 px-3">
            ...
            <button className='basis-1/3 text-gray-500 flex items-center justify-center  hover:rounded-full  hover:bg-gray-100 border-gray-300 '>
...
<div id="defaultModal" tabindex="-1" aria-hidden="true" data-modal-show="true" class={`${!showModal ? 'hidden': ''} bg-slate-800 bg-opacity-90 flex justify-center items-center transition-all ease-in-out top-0 right-0 bottom-0 left-0 z-50 h-screen fixed`}>
    <div class={`relative p-4 w-full max-w-3xl h-full md:h-auto transition-all ease-in-out ${!showModal?'opacity-0 ':'opacity-100 '}`}>
       
        <div class="relative bg-white rounded-lg shadow">
            
            <div class="flex px-3 space-x-2 items-center rounded-t border-b">
          ...
          <section className={`${createPost && 'hidden'}`}>
           <div id="questions" className="px-4 py-5" >
              ...
              <div className="my-4 flex items-center">
                ...
                <div className="flex items-center text-gray-500">
                  <span className='text-sm flex mr-2'><ion-icon name="play"></ion-icon></span>
                    <div className="">
                    <button id="dropdownDefault" data-dropdown-toggle="dropdown" class=" bg-gray-200 rounded-full focus:outline-none font-medium text-sm px-3 py-1 text-center inline-flex items-center" type="button">
              ...
              <textarea id="message" rows="1" class="block p-2.5 w-full text-sm text-gray-900 border-b rounded-lg border-gray-300" placeholder={`Start your question with "What", "How", "Why", etc.`}></textarea>
           </div>
           {/* Search Component */}
<div className="h-36"></div>
            ...
            <div class="flex justify-end items-center rounded-b border-t space-x-2 py-2 px-4">
                  ...
                  <button onClick={()=>setShowModal(false)}  data-modal-toggle="defaultModal" type="button" class="text-gray-500 bg-white focus:outline-none hover:bg-gray-100 rounded-full py-2.5 px-5  text-sm font-medium  "><span className=' '>Cancel</span></button>
             
                  <button data-modal-toggle="defaultModal" type="button" class="text-white bg-blue-500 hover:bg-blue-800 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center">Post</button>
              </div>
          </section>
          ...
          <section className={`${askQuestion && 'hidden'}`}>
            <div id="createPost" className=" px-4 py-3" >
                ...
                <textarea id="message" rows="15" class="block p-2.5 w-full text-sm text-gray-900 rounded-lg placeholder:text-base focus:!ring-red-600 active:border-red-600 focus:border" placeholder={`Say something....`}></textarea>
            </div>
            {/* Search Component */}

            {/* Buttons */}
            <div class="flex justify-between items-center rounded-b border-t py-2 px-4">
              <div className="flex space-x-2 ">

                  <button data-modal-toggle="defaultModal" type="button" class="text-gray-500 bg-white focus:outline-none hover:border-blue-400 hover:border text-sm font-medium px-2 "><span className='text-2xl font-bold '><ion-icon name="text-outline"></ion-icon></span></button>
                  <button data-modal-toggle="defaultModal" type="button" class="text-gray-500 bg-whitefocus:outline-none hover:border-blue-400 hover:border text-sm font-medium px-2 "><span className='text-2xl font-bold '><ion-icon name="images-outline"></ion-icon></span></button>
              </div>
                  <button data-modal-toggle="defaultModal" type="button" class="text-white bg-blue-500 hover:bg-blue-800 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center">Post</button>
              </div>
          </section>
